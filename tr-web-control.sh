...
if [ $ID == rhel ] || [ $ID == fedora ] || [ $ID == rocky ] || [ $ID == centos ]; then
	DISTRIBUTION=1
...
#判断是否安装transmission及安装版本
check_package(){
if command -v transmission-daemon >/dev/null 2>&1 ; then
        TR_VERSION=$(transmission-show -V 2>&1 | awk -F " " '{print $2}')
        BIG_TR_VERSION=$(echo $TR_VERSION | awk -F . '{print $1}')
	TR_MSG=$(echo "transmission 已安装,版本 $TR_VERSION")
	#根据版本定义安装目录
	if [ $BIG_TR_VERSION -le 3 ] ;then
		WEB="web"
	elif [ $BIG_TR_VERSION == 4 ]; then
		WEB="public_html"
	fi
else    
        TR_MSG=$(echo -e "\r${SHAN} transmission 未安装\r${RES}")
        exit 1  
fi
}

check_package
#获取最新transmission-web-control版本信息
getLatestReleases(){
	CONTROL_VERSION=`wget -O - https://api.github.com/repos/ronggang/transmission-web-control/releases/latest | grep tag_name | head -n 1 | awk -F '"' '{print $4}'`
}
getLatestReleases 2> /dev/null
...
MSG_BEGIN="开始"
MSG_BEGIN_TIME_OUT="如果下载慢，可以先下载，然后将安装包放在$TMP_FOLDER"
MSD_DOWNLOAD_URL="下载链接$DOWNLOAD_URL"
MSG_END="结束"
MSG_MAIN_MENU="
	欢迎使用   Transmission Web Control 中文安装脚本。
	官方帮助文档 : https://github.com/ronggang/transmission-web-control/wiki 
	最新脚本版本 : $CONTROL_VERSION
	$TR_MSG

	1. 安装最新的发布版本 ($CONTROL_VERSION)；
	2. 安装指定版本，可用于降级；
	3. 恢复到官方 UI；
...
initValues() {
	...
	# 判断是否指定了版本
	if [ "$CONTROL_VERSION" != "" ]; then
		# master 或 hash
		if [ "$CONTROL_VERSION" = "master" -o ${#VERSION} = 40 ]; then
			PACK_NAME="$CONTROL_VERSION.tar.gz"
		# 是否指定了 v
		...
		showLog "$MSG_SPECIFIED_VERSION $CONTROL_VERSION"
...
install() {
	# 是否指定版本
	if [ "$CONTROL_VERSION" != "" ]; then
		showLog "$MSG_TRY_SPECIFIED_VERSION $CONTROL_VERSION"
		# 下载安装包
		download
		# 解压安装包
		unpack
		...
		# 复制文件到
		cp -r $TMP_FOLDER/trans*/src/. $WEB_FOLDER/
		# 设置权限
...
download() {
	# 切换到临时目录
	cd "$TMP_FOLDER"
	FILE=transmission-web-control*.tar.gz
	# 判断安装包文件是否已存在
	if [ -f $FILE ]; then
		if [ $AUTOINSTALL = 0 ]; then
			echo -n "$PACK_NAME $MSG_PACK_IS_EXIST"
			read flag
		else
			flag="y"
		fi
...
unpack() {
	...
	if [ "$1" != "" ]; then
		tar -xzf transmission-web-control*.tar.gz -C "$1"
	else
		tar -xzf transmission-web-control*.tar.gz
	fi
...
getTransmissionPath() {
	...
	if [ $DISTRIBUTION == 1 ] || [ $DISTRIBUTION == 2 ]; then
...
#	if [ ! -d "$ROOT_FOLDER" ]; then
#		showLog "$MSG_FIND_WEB_FOLDER_FROM_PROCESS" 
